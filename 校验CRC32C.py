...
def calc(task_file: str) -> dict:
    global progress_dict
    global file_error,file_corrupted,file_passed
    global possible_moved_file_dict

    crc32_to_be  = rec_file_dict[task_file]
    if task_file in file_disappear:
        with possible_moved_file_dict_lock:
            if crc32_to_be in possible_moved_file_dict:
                possible_moved_file_dict[crc32_to_be].append(copy(task_file))
            else:
                possible_moved_file_dict[crc32_to_be] = [copy(task_file)]
        #打上完成时间戳
        progress_dict[task_file][2] = time.time()
        return {task_file: [crc32_to_be , "NotExist" , "【文件不存在】"]}
    ...
    try:
        ...
        if crc32_result == crc32_to_be:
            output = "【通过】"
            with file_passed_lock:
                file_passed += 1
        elif crc32_to_be == "NOT_IN_REC":
            output = "【不在记录上的文件】"
            with possible_moved_file_dict_lock:
                if crc32_result in possible_moved_file_dict:
                    possible_moved_file_dict[crc32_result].append(copy(task_file))
                else:
                    possible_moved_file_dict[crc32_result] = [copy(task_file)]
        else:
            output = "【已被修改】"
            with file_corrupted_lock:
...
for rec_file in possible_rec_files:
    if os.path.exists(rec_file):
        try:
            with open(file = rec_file ,mode="rt",encoding="utf-8-sig") as src:
                line_buf = (src.read()).splitlines()
        except Exception as e:
            handle_error(f"打开记录文件【{rec_file}】失败，详情：{e}。")
        else:
            break

if not line_buf:
    handle_error("找不到记录文件CRC32C校验值的记录文件，或记录文件为空。")

rec_file_dict = dict()
for line in line_buf:
    tmp = line.split(">>>>>")
    if (len(tmp) != 2) or (len(tmp[-1]) != 10) or (not tmp[-1].isdigit()):
        handle_error("记录文件的记录格式错误")
    rec_file_dict[tmp[0]] = tmp[-1]


os.system("title 比对当前目录下所有文件的路径中")
# 获取当前实际的文件列表（集合）
...
# 处理多出来的文件
file_not_in_record = {i for i in current_file_set if i not in rec_file_dict}
for i in file_not_in_record:
    rec_file_dict[copy(i)] = "NOT_IN_REC"
# 处理已经不存在的文件
file_disappear = {i for i in rec_file_dict if i not in current_file_set}

# 初始化进度信息
for file in rec_file_dict:
    try:
...
if file_corrupted:
    ...
    print_out_head += "".join(["已经被修改的文件：" , "\n    " , "\n    ".join(file_corrupted) , "\n\n"])
    del file_corrupted

movement_list = []
for k,v in possible_moved_file_dict.items():
    if len(v)>1:
        move_src_filelist = [file for file in v if file in file_disappear]
        move_dst_filelist = [file for file in v if file in file_not_in_record]
        # 排除单独缺失或多余几个相同文件的情况
        if move_src_filelist and move_dst_filelist:
            # 从记录中删去，现在归为了“被移动”的那一类
            [file_disappear.discard(file) for file in move_src_filelist]
            [file_not_in_record.discard(file) for file in move_dst_filelist]
            # 从结果字典里修改原来的消失、多余为“疑似被移动”
            for file in (move_src_filelist + move_dst_filelist):
                result_dict[file][-1] = "【疑似被移动】"
            # 生成打印的结果
            movement_list.append(
                "".join([
                    ...
                    f"CRC32C为【{k}】的文件：" , 
                    "\n    " ,
                    " ; ".join([f"【{i}】" for i in move_src_filelist]),
                    "  --->>>  ",
                    " ; ".join([f"【{i}】" for i in move_dst_filelist]),
                    "\n",
                ])
            )
del possible_moved_file_dict
if movement_list:
    need_result_file = True
    print_out_head += "检测到缺失的文件（不存在）与多余（不在记录上）的文件中，\n有一对或多对CRC32C校验值相同的文件，猜测文件被移动的情况如下（箭头指向移动后的位置）：\n"
    print_out_head += "".join(movement_list)
    print_out_head += "\n\n"
    del movement_list

if file_disappear:
    need_result_file = True
    print_out_head += "".join(["排除文件移动后，缺失的文件（不存在）：" , "\n    " , "\n    ".join(file_disappear) , "\n\n"])
    del file_disappear

if file_not_in_record:
    need_result_file = True
    print_out_head += "".join(["排除文件移动后，多余的文件（不在记录上）：" , "\n    " , "\n    ".join(file_not_in_record) , "\n\n"])
    del file_not_in_record

if file_error:
    need_result_file = True
    print_out_head += "".join(["打开时出错的文件：" , "\n    " , "\n    ".join(file_error) , "\n\n"])
    del file_error

result_list = [([k]+v) for k,v in result_dict.items()]
del result_dict
result_list.sort(key=lambda x:x[0])
write_out = print_out_head + \
    "\n".join(
...
if need_result_file:
    try:
        pre_allocate_write_output_file(output_path=verify_output_file , data=write_out , encoding="utf-8-sig")
    except Exception as e:
        handle_error(f"校验结果写入出错，详情：{e}。")
del write_out
...
sys.stdout.write(
    "\n".join([
        ...
        "【 处理完成 】",
        "",
        (f"校验结果已写入目标文件夹下的【{verify_output_file}】文件中。"  if  need_result_file  else  ""),
        "",
        "★  按任意键结束  ★",
        "\n",
    ])
